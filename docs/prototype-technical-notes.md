...
Use `?oceanValidation=<scenario>&foam=0|1&seed=<number>` to load a fixed high-quality validation state. The matrix includes nine deck scenarios (`rail-low|medium|high`, `bow-low|medium|high`, `bridge-low|medium|high`) plus `aerial-clear-50`, `aerial-clear-150`, and `aerial-storm-300` to catch altitude-dependent LOD regressions. They fix camera, atmosphere, Beaufort, world time and simulation time. Runtime metrics and spectrum estimates are exposed as `window.__oceanValidation` for capture tooling. WebGPU timestamp queries are resolved asynchronously and shown in the debug panel.
